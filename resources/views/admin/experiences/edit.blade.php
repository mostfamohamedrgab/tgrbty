...
<section class="content">
    ...
    <div class="form-group">
      <label >صوره مصغره
        <img id="blah" src="{{asset('public/storage/imgs/'.$experience->img)}}"
            style="width:70px;height:70px;border-radius:50%;margin:10px" />
      </label>
      <input id="imgInp" type="file" accept="image/*" class="form-control" name="img" >
    </div>

    <div class="form-group">
      <label>المحتوي</label>
      <textarea name="content">{{ $experience->content }}</textarea>
        <script>
    ...
    <div class="form-group">
      ...
      <select class="form-control" required="required" name="user_id">
        @foreach($users as $user)
          <option
          {{$user->id == $experience->user_id ? 'selected' : ''}}
    ...
    <div class="form-group">
      ...
      <select class="form-control" required="required" name="section_id">
        @foreach($sections as $section)
          <option
          {{$user->id == $experience->section_id ? 'selected' : ''}}
